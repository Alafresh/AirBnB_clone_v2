...
def do_deploy(archive_path):
    ...
    run('tar -xzf /tmp/{} -C /data/web_static/releases/{}'.format(archive_path, tmp))
    run('rm /tmp/{}'.format(archive_path))
    run('mv /data/web_static/releases/{}/web_static/* /data/web_static/releases/{}/'.format(tmp, tmp))
    run("rm -rf /data/web_static/releases/{}/web_static".format(tmp))
    run('rm -rf /data/web_static/current')
    run('ln -s /data/web_static/releases/{}/ /data/web_static/current'.format(tmp))
